...
### `run`과 `dir`은 Claude 전용

세션 모드는 끝까지 Claude Code의 모양이다 — 프로필 디렉터리, 그걸 가리키는 환경변수,
그 안으로 띄우는 바이너리, 안쪽 자격증명 배치, MCP 미러링. Codex 세션을 요청했는데
`claude`를 띄우는 것은 가능한 답 중 최악이라(엉뚱한 도구를 엉뚱한 계정으로 돌리면서
성공한 것처럼 보인다), **명시적으로 거절하고 대신 쓸 명령을 알려준다.**

Codex 세션 모드는 별개의 작업이다. `CODEX_HOME`이 `CLAUDE_CONFIG_DIR`의 대응물로
보이지만, seeding·공유·미러링 기계장치 전부를 다시 만들어야 한다.
