...
const (
	...
	// KindInvalidClient means cswap's OAuth client credential was rejected.
	...
	// blips, 5xx, and — deliberately — any 4xx body cswap could not parse a
...
const (
	...
	// effect, so the store cswap would write is not the one Claude Code reads.
	KindStoreUnmirrored ErrorKind = "store-unmirrored"

	// KindConsumeBusy means another cswap surface holds this slot's gate.
...
var errorNotes = map[ErrorKind]string{
	...
	KindInvalidClient:   "cswap's OAuth client was rejected — systemic, not this account",
	KindConsumeBusy:     "another cswap surface holds the slot — retries next pass",
